...
%
% Input:
% X     - epoched data, with size(X) = [n_samples_per_epoch_x, n_channels, n_epochs]
...
if not(isempty(X))
    [T_e, n_channels, N_e] = size(X);
elseif not(isempty(opt.Cxxe))
    [~,n_channels, N_e] = size(opt.Cxxe);
else
    error('Either X or Cxxe must be non-empty!');
end
if not(size(z,2) == N_e)
...
elseif isempty(opt.Cxx)
    ...
    X_tmp = reshape(permute(X, [1,3,2]), [T_e*N_e, n_channels]);
    Cxx = cov(X_tmp);
else
    Cxx = opt.Cxx;
end
...
% whiten the data
[~, M] = whiten_data(X, opt.pca_X_var_explained, Cxx);
