...
fprintf('\n--- Begin cross-validating mSPoC regularizers ---\n')

%% precompute some useful stuff
fprintf(' \nprecomputing whitening for Y   ')
...
for k=1:n_folds
    ...
    mspoc_params.Cyy = [];
    [~, My_tr{k}, Cyy_tr{k}] = util_mspoc__prepare_Y_signal(Y_tr, mspoc_params);
...
for ii=1:n_kappa_tau
    for jj=1:n_kappa_y
        fprintf(' \nstarting xval for kappa_tau = %g, kappa_y = %g ',kappa_tau_list(ii),kappa_y_list(jj))
        for k=1:n_folds
...
fprintf('\n--- Finished cross-validating mSPoC regularizers ---\n')
